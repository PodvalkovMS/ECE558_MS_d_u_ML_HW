...
def SAlgorithm(T,Th, alpha, numlter, task, map, links, traffic_all, g):
    ...
    while T>=Th:       
        ...
        for i in range(numlter):
            ...
            cost_old_links=FindTheCost(links_core, traffic_all, task_plas) # Calculate the cost function 
            ...
            cost_new_links=FindTheCost(link_core_new, traffic_all, task_old)  # calculate the new cost 
...
for i in range(0,3):     # For all traffic do
    ...
    final_cost=FindTheCost(links_final, traffic_matrix[i], task_finals)  # Calculate the cost of finale result 
